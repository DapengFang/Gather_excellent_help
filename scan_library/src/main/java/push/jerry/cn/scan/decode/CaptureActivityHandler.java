...
package push.jerry.cn.scan.decode;
...
import push.jerry.cn.scan.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
    ...
    public CaptureActivityHandler(CaptureActivity activity,
        ...
        ResultPointCallback resultPointCallback = new ViewfinderResultPointCallback(
                activity.getViewfinderView());
        // 启动扫描线程
        decodeThread = new DecodeThread(activity, decodeFormats, baseHints,
                characterSet, resultPointCallback);
        decodeThread.start();
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case ConstValue.scan_restart_preview: // 准备进行下一次扫描
                Log.d(TAG, "Got restart preview message");
                restartPreviewAndDecode();
                break;
            case ConstValue.scan_decode_succeeded:
                ...
                activity.handleDecode((Result) message.obj, barcode,
                        scaleFactor);
                break;
            case ConstValue.scan_decode_failed:
                // We're decoding as fast as possible, so when one decode fails,
                // start another.
                state = State.PREVIEW;
                cameraManager.requestPreviewFrame(decodeThread.getHandler(),
                        ConstValue.scan_decode);
                break;
            case ConstValue.scan_return_scan_result:
                Log.d(TAG, "Got return scan result message");
                activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
                activity.finish();
                break;
            case ConstValue.scan_launch_product_query:
    ...
    public void quitSynchronously() {
        ...
        Message quit = Message.obtain(decodeThread.getHandler(), ConstValue.scan_quit);
        quit.sendToTarget();
        ...
        removeMessages(ConstValue.scan_decode_succeeded);
        removeMessages(ConstValue.scan_decode_failed);
    }
